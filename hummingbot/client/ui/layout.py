...
from abquantui.commands.help_command import HEADER
...
def get_version(param: Dict):
    return [("class:header", f"Version: {param.get('version', 'None')}")]


def get_active_strategy(param: Dict):
    style = "class:log-field"
    return [(style, f"Strategy: {param.get('strategy_name', 'None')}")]
def get_config_file(param: Dict):
    style = "class:log-field"
    return [(style, f"Config File: {param.get('config_file', 'None')}")]
def get_log_file(param: Dict):
    style = "class:log-field"
    return [(style, f"Log File: {param.get('log_file', 'None')}")]
...
def get_strategy_file():
    style = "class:log-field"
    return [(style, f"Strategy File: filename")]
...
def generate_layout_2(input_field: TextArea,
                      output_field: TextArea,
                      log_field: TextArea,
                      search_field: SearchToolbar,
                      right_pane_toggle: Button,
    ...
    components["item_top_version"] = Window(FormattedTextControl(get_version(kwargs)), style="class:header")
    components["item_top_active"] = Window(FormattedTextControl(get_active_strategy(kwargs)), style="class:header")
    components["item_top_file"] = Window(FormattedTextControl(get_config_file(kwargs)), style="class:header")
    components["item_top_log"] = Window(FormattedTextControl(get_log_file(kwargs)), style="class:header")
    components["item_top_toggle"] = right_pane_toggle
    components["pane_top"] = VSplit([components["item_top_version"],
                                     components["item_top_active"],
                                     components["item_top_file"],
                                     components["item_top_log"],
                                     components["item_top_toggle"]], height=1)
    ...
    components["pane_right_top"] = VSplit(tab_buttons, height=1, style="class:log-field", padding_char=" ", padding=2)
    components["pane_right"] = ConditionalContainer(
        Box(body=HSplit([pane_right_field, search_field], width=Dimension(weight=1)),
            padding=0, padding_left=2, style="class:log-field"),
        filter=True
    )
    components["hint_menus"] = [Float(xcursor=True,
